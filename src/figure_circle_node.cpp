...
#define FLIGHT_ALTITUDE 0.7
#define RATE            100 // 频率 hz
#define CIRCLE_RADIUS   1.5 // 圆的半径 m
#define CIRCLE_DURATION 10  // 完成一圈所花费的时间
...
void init_path()
{
    ...
    for (int i = 0; i < STEPS; i++) {
        ...
        path[i].position.x = CIRCLE_RADIUS * sin(theta);
        path[i].position.y = CIRCLE_RADIUS * cos(theta) - CIRCLE_RADIUS;
        path[i].position.z = FLIGHT_ALTITUDE;

        path[i].velocity.x = VELOCITY_MAGNITUDE * cos(theta);
        path[i].velocity.y = -VELOCITY_MAGNITUDE * sin(theta);
        path[i].velocity.z = 0;
        
        path[i].acceleration_or_force.x = -VELOCITY_MAGNITUDE * sin(theta);
        path[i].acceleration_or_force.y = -VELOCITY_MAGNITUDE * cos(theta);
...
// #define FLIGHT_ALTITUDE 1
// #define RATE            100 // 频率 hz
// #define CIRCLE_RADIUS   2 // 圆的半径 m
// #define CIRCLE_DURATION 20  // 完成一圈所花费的时间
// #define STEPS           (CIRCLE_DURATION * RATE)
// #define VELOCITY_MAGNITUDE (2.0 * M_PI * CIRCLE_RADIUS / CIRCLE_DURATION)
...
//         // path[i].yaw = 2 * M_PI * i / STEPS + M_PI / 2;
//         // if (path[i].yaw >= 2 * M_PI) {
//         //     path[i].yaw -= 2 * M_PI;
//         // }
//     }
// }
...
//         rate.sleep();
//         if(current_state.mode == "OFFBOARD" && current_state.armed) {
//             std::cout << "Current mode: " << current_state.mode << " Arm state:" << current_state.armed << std::endl;
//             break;
//         }
//         else {
//             std::cout << "Current mode: " << current_state.mode << " Arm state:" << current_state.armed << std::endl;
