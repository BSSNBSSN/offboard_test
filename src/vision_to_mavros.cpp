...
void vins_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
    if(msg->header.frame_id == "world")
    {
        ...
        // Eigen::Vector3d ypr = uav_utils::quaternion_to_ypr(q_mav);
        // q_mav = uav_utils::ypr_to_quaternion(Eigen::Vector3d(ypr(0), -ypr(2), ypr(1)));
    }
}
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        vision.pose.position.x = p_mav[0] / 1000;
        vision.pose.position.y = p_mav[1] / 1000;
        vision.pose.position.z = p_mav[2] / 1000;
